...
namespace MissileETA_Engine
{
    internal static class HudLabelBackdrop
    {
        ...
        internal static void FitBackdropToText(Image backdrop, Text text, Color accentColor)
        {
            ...
            float alpha = Mathf.Clamp01(MissileEtaPlugin.LabelBackgroundAlpha.Value);
        ...
        {
            return Mathf.Max(10, Mathf.RoundToInt(HudScreenScale.Px(basePx * MissileEtaPlugin.LabelFontScale.Value)));
        }

        internal static float VerticalOffsetPx()
        {
            return HudScreenScale.Px(MissileEtaPlugin.LabelVerticalOffsetPx.Value);
        }
    }
}
