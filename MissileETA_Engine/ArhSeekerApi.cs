...
namespace MissileETA_Engine
{
    ...
    internal static class ArhSeekerApi
    {
        ...
        internal static ArhDisplaySample ComputeDisplay(Missile missile, Unit target)
        {
            ...
            float sec = (dist - terminalRange) / Mathf.Max(closure, MissileEtaPlugin.MinClosureMps.Value);
            if (sec <= 0f || sec > MissileEtaPlugin.MaxEtaSeconds.Value)
                return sample;
        ...
        private static float GetTerminalRange(Missile missile)
        {
            if (_terminalRangeField == null)
                _terminalRangeField = AccessTools.Field(typeof(ARHSeeker), "terminalRange");
