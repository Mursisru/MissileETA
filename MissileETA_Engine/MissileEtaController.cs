...
namespace MissileETA_Engine
{
    internal sealed class MissileEtaController : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            float hz = MissileEtaPlugin.UpdateHz.Value;
            if (hz > 0f)
            {
            ...
            float edgeMargin)
        {
            for (int i = 0; i < _ownMissiles.Count; i++)
            {
                ...
                if (target == null && !(MissileEtaPlugin.ShowArhCountdown.Value && (noTargetLaunch || (isArh && hardLost))))
                {
                    MissileEtaDiagLog.VerboseLine(
                ...
                if (target != null)
                {
                    ...
                    if (!TryFormatEtaText(
                        etaState,
                        raw,
                        MissileEtaPlugin.OwnDecimalPlaces.Value,
                ...
                if (MissileEtaPlugin.ShowArhCountdown.Value)
                {
                    string prefix = MissileEtaPlugin.ArhPrefix.Value ?? "R";
                ...
                if (!TryBuildSlot(
                    ...
                    margin,
                    edgeMargin,
                    MissileEtaPlugin.ShowOwnOffScreenArrows.Value,
                    true,
                    etaText,
            ...
            float edgeMargin)
        {
            for (int i = 0; i < _incomingMissiles.Count; i++)
            {
                ...
                if (!TryFormatEtaText(
                    etaState,
                    raw,
                    MissileEtaPlugin.IncomingDecimalPlaces.Value,
                    filterSettings.DisplayQuantizeStep,
                    out string etaText))
                {
                    MissileEtaDiagLog.VerboseLine(
                ...
                if (!TryBuildSlot(
                    ...
                    margin,
                    edgeMargin,
                    MissileEtaPlugin.ShowOffScreenArrows.Value,
                    false,
                    etaText,
                    out MissileLabelSlot slot,
                    tag))
